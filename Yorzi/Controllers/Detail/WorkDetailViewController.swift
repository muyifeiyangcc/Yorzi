...
final class WorkDetailViewController: BaseViewController {
    ...
    private let likeCountLabel = UILabel.appLabel(size: 13, color: AppTheme.secondary)
    ...
    override func viewDidLoad() {
        ...
        // Authors always have access to the original media of their own work.
        unlocked = work.authorID == DataRepository.shared.currentUserID || DataRepository.shared.isResolutionUnlocked(for: work.id)
        buildPage()
    ...
    private func authorHeader() -> UIView {
        ...
        let name = UILabel.appLabel(user?.name ?? "Creator", size: 14, weight: .semibold); let role = UILabel.appLabel(work.location, size: 11, color: AppTheme.secondary)
    ...
    private func titleSection() -> UIView {
        ...
        let currentWork = DataRepository.shared.works.first(where: { $0.id == work.id }) ?? work
        likeCountLabel.text = "\(currentWork.likes)"
        let stats = UIView(); let comments = UILabel.appLabel("\(commentCount)", size: 13, color: AppTheme.secondary); let commentIcon = UIImageView(image: UIImage(systemName: "bubble.left")); commentIcon.tintColor = AppTheme.secondary; likeButton.setImage(UIImage(systemName: currentWork.liked ? "heart.fill" : "heart"), for: .normal); likeButton.tintColor = currentWork.liked ? AppTheme.destructive : AppTheme.ink; if !likeActionInstalled { likeActionInstalled = true; likeButton.addAction(UIAction { [weak self] _ in guard let self else { return }; DataRepository.shared.toggleLike(self.work.id); let current = DataRepository.shared.works.first { $0.id == self.work.id }; self.likeButton.setImage(UIImage(systemName: current?.liked == true ? "heart.fill" : "heart"), for: .normal); self.likeButton.tintColor = current?.liked == true ? AppTheme.destructive : AppTheme.ink; self.likeCountLabel.text = "\(current?.likes ?? 0)" }, for: .touchUpInside) }; [likeButton, likeCountLabel, commentIcon, comments].forEach(stats.addSubview); likeButton.snp.makeConstraints { $0.leading.centerY.equalToSuperview(); $0.size.equalTo(20) }; likeCountLabel.snp.makeConstraints { $0.leading.equalTo(likeButton.snp.trailing).offset(4); $0.centerY.equalToSuperview() }; commentIcon.snp.makeConstraints { $0.leading.equalTo(likeCountLabel.snp.trailing).offset(18); $0.centerY.equalToSuperview(); $0.size.equalTo(16) }; comments.snp.makeConstraints { $0.leading.equalTo(commentIcon.snp.trailing).offset(4); $0.centerY.equalToSuperview() }; stats.snp.makeConstraints { $0.height.equalTo(22) }
    ...
    private func creditRow(_ value: String) -> UIView { let row = UIView(); row.snp.makeConstraints { $0.height.equalTo(30) }; let label = UILabel.appLabel(value, size: 11); let button = UIButton(type: .system); button.setTitle("View profile", for: .normal); button.setTitleColor(AppTheme.secondary, for: .normal); button.titleLabel?.font = .systemFont(ofSize: 10); row.addSubview(label); row.addSubview(button); label.snp.makeConstraints { $0.leading.centerY.equalToSuperview() }; button.snp.makeConstraints { $0.trailing.centerY.equalToSuperview() }; let name = value.components(separatedBy: " · ").first ?? value; let matched = DataRepository.shared.users.first(where: { $0.name == name }); let fallback = DataRepository.shared.visibleUsers.first; if let user = matched ?? fallback { button.addAction(UIAction { [weak self] _ in
            guard let self else { return }
            if user.id == DataRepository.shared.currentUserID {
                if let tabs = self.tabBarController as? MainTabBarController { tabs.selectTab(4) }
                else { self.tabBarController?.selectedIndex = 4 }
            } else {
                self.push(CreatorProfileViewController(userID: user.id))
            }
        }, for: .touchUpInside) }; let line = UIView(); line.backgroundColor = AppTheme.divider; row.addSubview(line); line.snp.makeConstraints { $0.leading.trailing.bottom.equalToSuperview(); $0.height.equalTo(1) }; return row }
    ...
    private func showCommentMore(_ comment: AppComment) { let sheet = CustomSheetView(items: ["Report", "Block", "Cancel"]) { [weak self] item in if item == "Report" { DataRepository.shared.report(comment.authorID, reason: "Comment") } else if item == "Block" { self?.blockUserAndReturnToRoot(comment.authorID) } }; sheet.present(in: view.window ?? view) }
    ...
    private func showMore() { let sheet = CustomSheetView(items: ["Report", "Block", "Cancel"]) { [weak self] item in if item == "Report", let id = self?.work.authorID { self?.push(ReportViewController(targetID: id)) } else if item == "Block", let id = self?.work.authorID { self?.blockUserAndReturnToRoot(id) } }; sheet.present(in: view.window ?? view) }
